...
// }else{
	...
	try{
		...
		if($name_check->rowCount()!=0){ //地標名稱重複
			...
			$sql_tr = "insert into lm value(null, 1, 1, :LM_branch, :LM_station, :LM_name, :LM_type, 0, :LM_description, :LM_phone, :LM_cellphone, :LM_address, :LM_longitude, :LM_latitude, :LM_opentime, :LM_avgcost, :LM_staycost1, :LM_staycost2, :LM_staycost4, :LM_staycostadd1, :LM_adultcost, :LM_childcost, '', '', '', '', '', '')";
			$statment = $pdo->prepare($sql_tr);
