<?php 
$trNo=json_decode($_REQUEST["TR_no"]);
try {
	...
	if($trNo->TR_type==5){
		...
		while($lmtrRow=$lmtrItem->fetch(PDO::FETCH_ASSOC)){  //該天的地標行程編號的地標編號
			...
			$str.="<span class='plBdot'><div class='ptourName'>".$lmNameRow["LM_name"]."</div></span>";
			$str.="<span class='plBline'></span>";
		} //$lmtrRow
			$str=substr($str,0,-29);
			$str.="</li>";
		}//$lmtrdayRow
